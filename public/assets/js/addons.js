...
require.config({
    paths: {
        'jquery-colorpicker': '../addons/flbooth/js/jquery.colorpicker.min',
        'jquery-autocomplete': '../addons/flbooth/js/jquery.autocomplete',
        'jquery-jqprint': '../addons/flbooth/js/jquery.jqprint-0.3.min',
		'jquery-migrate': '../addons/flbooth/js/jquery.migrate-1.2.1.min',
		'vue': '../addons/flbooth/js/vue.min',
		'chat': '../addons/flbooth/js/vue.min',
		'sortablejs': '../addons/flbooth/js/Sortable.min',
		'vuedraggable': '../addons/flbooth/js/vuedraggable.umd.min',
		'clipboard': '../addons/flbooth/js/clipboard.min'
    },
    shim: {
        'jquery-colorpicker': {
            deps: ['jquery'],
		...
		'chat': {
		    deps: ['css!../addons/flbooth/css/chat.css'],
		    exports: '$.fn.extend'
		},
        'sortablejs': {
            deps: ['jquery'],
			...
			methods: {
					...
					Fast.api.ajax({
						...
						if(data === 0){
							Layer.open({
								...
								area: '400px;',
								shade: 0,
								id: 'flbooth_v1.1.1', //设定一个id，防止重复弹出
								resize: false,
								btn: ['确认', '关闭'],
								btnAlign: 'c',
								...
								content: `<div style="padding: 50px 40px; line-height: 22px; background-color: #222d32; color: #fff; font-weight: 300;">
									...
									恭喜你，已经升级到 flbooth V1.1.1 版本，现需要对原有数据进行升级，
							...
							var sound = new Audio();
							sound.src = Fast.api.cdnurl('/assets/addons/flbooth/voice/open.mp3');
							sound.play();
						}
						return false;
					});
				...
				playAudio(type, str){
					...
					if(type == 'main'){
						...
						url = this.cdnurl('/assets/addons/flbooth/voice/open.mp3');
					}else if(type == 'open'){
						url = this.cdnurl('/assets/addons/flbooth/voice/chat.mp3');
					}
				    sound.src = url;
				    sound.play();
				},
				...
				toFind(type){
					...
					Fast.api.open('/index/flbooth.find/add.html?type='+type, name);
				},
				full(){
