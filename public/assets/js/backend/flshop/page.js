define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'upload', 'vue', 'vuedraggable'], function ($, undefined, Backend, Table, Form, Upload, Vue, wanldraggable) {
	var Controller = {
		...
		template: function () {
		    ...
		    Table.api.init({
		        extend: {
		            index_url: 'flbooth/page/template' + location.search,
		            add_url: 'flbooth/page/add/type/template',
		            edit_url: 'flbooth/page/edit',
		            del_url: 'flbooth/page/del',
		            multi_url: '',
		            table: 'flbooth_page',
			...
			$(document).on("click", ".btn-publish", function () {
				Fast.api.ajax({
				    url: "flbooth/page/operate",
				    data: {
						'ids': $(this).data('id'),
						'type': 'index'
					}
				}, function(data, ret){
					Backend.api.addtabs('flbooth/page','页面管理');
				});
			});
			...
			var vm = new Vue({
						...
						module: {
							"Basics": [{
								...
								},
								{
									...
									"data": [{
										"image": "/assets/addons/flbooth/img/page/article-default.png",
										"tips": "建议尺寸750x360",
										"articleLink": ""
									}]
								},
				...
				methods: {
					...
					publishTpl() {
						let _this = this;
						Fast.api.ajax({
							url: 'flbooth/page/operate/type/systpl',
							data: _this.pageData,
						}, function(data, ret){
							Backend.api.addtabs('flbooth/page/template','模板管理');
						});
					},
					// 恢复历史记录
					recover(id) {
					...
					obtainLink(key, num, type, multiple){
						parent.Fast.api.open("flbooth/link/select?multiple=" + multiple, __('选择链接'), {
							area: ['800px', '600px'],
					...
					categoryLink(key, num, multiple){
						parent.Fast.api.open("flbooth/category/select?multiple=" + multiple, __('选择类目链接'), {
							area: ['800px', '600px'],
						    callback: function (data) {
					...
					advertLink(key, num, type, multiple){
						parent.Fast.api.open("flbooth/advert/select?multiple=" + multiple, __('选择展示广告'), {
							area: ['800px', '600px'],
						    callback: function (data) {
					...
					articleLink(key, num, type, multiple){
						parent.Fast.api.open("flbooth/article/select?multiple=" + multiple, __('选择文章'), {
							area: ['800px', '600px'],
						    callback: function (data) {
								Vue.set(vm.pageData.item[key].data[num], type, data.id);	
