				...
				methods: {
					...
					shopRefuse(index) {
						...
						layer.prompt({title: '请输入拒绝入驻理由', formType: 2}, function(text, key){
							Fast.api.ajax({
								url: `flbooth/auth/refuse/ids/${app.shopAuthList[index].id}`,
								data: {
									row: {
										refuse: text
									}
					...
					refundRefuse(index) {
						...
						layer.prompt({title: '请输入拒绝退款理由', formType: 2}, function(text, key){
							console.log(text);
							Fast.api.ajax({
								url: `flbooth/refund/refuse/ids/${app.refundList[index].id}`,
								data: {
									row: {
										refund_content: text
									}
					...
					shopDetails(index) {
						Fast.api.open(`flbooth/auth/detail/ids/${this.shopAuthList[index].id}`, "查看入驻信息", {});
					},
					refundDetails(index) {
						Fast.api.open(`flbooth/refund/detail/ids/${this.refundList[index].id}`, "查看退款", {});
					}
				}
			});
		},
