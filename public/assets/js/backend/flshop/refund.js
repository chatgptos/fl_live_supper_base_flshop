define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function($, undefined, Backend, Table, Form) {
	...
	var Controller = {
		index: function() {
			...
			Table.api.init({
				extend: {
					index_url: 'flbooth/refund/index' + location.search,
					add_url: '',
					edit_url: '',
					del_url: '',
					multi_url: '',
					table: 'flbooth_refund',
				}
			});
			...
			table.bootstrapTable({
				...
				columns: [
					[
						...
						{field: 'operate',title: __('Operate'),table: table,events: Table.api.events.operate,buttons: [{name: 'detail',title: __('查看退款'),classname: 'btn btn-xs btn-info btn-dialog',icon: 'fa fa-eye',text: '查看退款',url: 'flbooth/refund/detail'},{name: 'agree',title: __('平台介入判定退款合理'),classname: 'btn btn-xs btn-success btn-magic btn-ajax',icon: 'fa fa-check',text: '退款合理',confirm: '确认同意买家退款，款项会直接转到用户余额？',url: 'flbooth/refund/agree',visible: function(row) {if (row.state == 3) {return true;}},success: function(data, ret) {table.bootstrapTable('refresh');return false;},error: function(data, ret) {console.log(data, ret);Layer.alert(ret.msg);return false;}},{name: 'refuse',title: __('平台介入判定退款不合理'),classname: 'btn btn-xs btn-danger btn-dialog',icon: 'fa fa-times',text: '退款不合理',url: 'flbooth/refund/refuse',visible: function(row) {if (row.state == 3) {return true;}},extend: 'data-area=["500px","270px"]'}],formatter: Table.api.formatter.operate}
					]
				]
			});
		...
		detail: function () {
		 //    $(document).on("click", ".btn-agree", function () {
			// 	Fast.api.ajax({
			// 		url:'flbooth/refund/agree?ids='+Backend.api.query('ids'),
			// 	}, function(data, ret){
			// 		Layer.closeAll();
			// 		return false;
			// 	});
