    ...
    var Controller = {
        index: function () {
            ...
            var menuUpdate = function () {
                $.post("flbooth/wechat/menu/edit", {menu: JSON.stringify(getMenuList())}, function (data) {
                    if (data['code'] == 1) {
                    } else {
            ...
            $(document).on('click', "#menuRemote", function () {
                Layer.confirm("确认加载远程微信服务端菜单？<br>本地菜单将会被覆盖!!!", {icon: 3}, function () {
                    $.post("flbooth/wechat/menu/remote", {}, function (ret) {
            ...
            $(document).on('click', "#menuSyn", function () {
                $.post("flbooth/wechat/menu/sync", {}, function (ret) {
                    var msg = ret.hasOwnProperty("msg") && ret.msg != "" ? ret.msg : "";
                    if (ret.code == 1) {
