define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'vue'], function ($, undefined, Backend, Table, Form, Vue) {
    var Controller = {
		bill: function () {
		    ...
		    Table.api.init({
		        extend: {
		            index_url: 'flbooth/finance/bill' + location.search,
		            add_url: '',
		            edit_url: '',
		            del_url: '',
		            multi_url: '',
		        ...
		        columns: [
		            [
                        ...
                        {field: 'operate', title: __('Operate'), table: table, events: Table.api.events.operate,buttons: [
							...
							icon: 'fa fa-eye',url: 'flbooth/finance/billDetail'},
							{
								name: 'type',
								title: function (row) {
								...
								url: function (row) {
									var url = '链接异常';
									switch(row.type) {
									    case "pay": url = 'flbooth/order/detail/order_no'; break; // 商品交易
										case "groups": url = 'flbooth/groupsorder/detail/order_no'; break; // 拼团交易
										case "withdraw": url = 'flbooth/finance/withdrawDetail/ids'; break; // 提现
										case "refund": url = 'flbooth/refund/detail/order_no'; break; // 退款 1.1.3升级
									}
			...
			$(".btn-paypal").click(function() {
				Fast.api.open(`flbooth/finance/payment`, `提现`, {
					area: ["420px", "720px"],
					callback: res =>{
		...
		withdraw: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'flbooth/finance/withdraw' + location.search,
                    add_url: '',
                    edit_url: '',
                    del_url: '',
                    multi_url: '',
                ...
                columns: [
                    [
						...
						{field: 'operate', title: __('Operate'), table: table, events: Table.api.events.operate,buttons: [
							...
							classname: 'btn btn-xs btn-info btn-dialog',icon: 'fa fa-eye',url: 'flbooth/finance/withdrawDetail'}
						],formatter: Table.api.formatter.operate}
                    ]
                ]
            });
			...
			$(".btn-bankuser").click(function() {
				Fast.api.open(`flbooth/finance/user`, `提现账户`, {
					callback: res =>{
						table.bootstrapTable('refresh', {});
					}
				});
		...
		payment: function () {
			var vm = new Vue({
				...
				methods: {
					choiceUser(){
						parent.Fast.api.open("flbooth/finance/user?multiple=true", __("提现账户"), {
							callback: res => {
								this.bankData = res;
							}
						});
		...
		user: function () {
			...
			Table.api.init({
			    extend: {
			        index_url: 'flbooth/finance/user' + location.search,
			        add_url: 'flbooth/finance/userAdd',
			        edit_url: 'flbooth/finance/userEdit',
			        del_url: 'flbooth/finance/userDel',
			        multi_url: '',
			        table: 'flbooth_page',
			    }
			});
