define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'template', 'jquery-jqprint', 'jquery-migrate'], function ($, undefined, Backend, Table, Form, Template, Jqprint, Migrate) {
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'flbooth/groupsorder/index' + location.search,
                    add_url: '',
                    edit_url: '',
                    del_url: 'flbooth/groupsorder/del',
                    multi_url: 'flbooth/groupsorder/multi',
                    table: 'flbooth_order',
                }
            });
            var table = $("#table");
			...
			$(document).on("click", ".detail[data-id]", function () {
			    Backend.api.open('flbooth/groupsorder/detail/id/' + $(this).data('id'), __('查看详情'),{area:['1200px', '780px']});
			});
			//查看退款
			$(document).on("click", ".refund[data-id]", function () {
			    Backend.api.open('flbooth/refund/detail/ids/' + $(this).data('id'), __('查看退款'));
			});
			// 查看评论
			$(document).on("click", ".comment[data-id]", function () {
			    Backend.api.open('flbooth/comment/detail/order_id/' + $(this).data('id'), __('查看评论'));
			});
			// 查看退款
			$(document).on("click", ".btn-selected", function () {
			    Backend.api.open('flbooth/refund/detail/order_id/' + $(this).data('id'), __('查看退款'));
			});
			// 即时沟通
			...
			$(document).on("click", ".btn-delivery", function () {
				if($(this).data('id')){
					Backend.api.open('flbooth/groupsorder/delivery/ids/' + $(this).data('id'), __('发货'),{area:['1000px', '700px']});
				}else{
					Backend.api.open('flbooth/groupsorder/delivery/ids/' + Table.api.selectedids(table), __('批量发货'),{area:['1000px', '700px']});
				}
			});
			// 打印 & 批量打印订单 自动关闭窗口parent.Layer.closeAll();
			$(document).on("click", ".btn-invoice", function () {
				if($(this).data('id')){
					Backend.api.open('flbooth/groupsorder/invoice/ids/' + $(this).data('id'), __('查看发货单'),{area:['1100px', '750px']});
				}else{
					Backend.api.open('flbooth/groupsorder/invoice/ids/' + Table.api.selectedids(table), __('批量查看发货单'),{area:['1100px', '750px']});
				}
			});
			// 查询物流状态
			$(document).on("click", ".kuaidisub[data-id]", function () {
			    Backend.api.open('flbooth/groupsorder/relative/id/' + $(this).data('id'), __('快递查询'),{area:['800px', '600px']});
			});
			// 提交云面单
		...
		comment: function () {
			...
			Table.api.init({
			    extend: {
			        index_url: 'flbooth/comment/index' + location.search,
			        add_url: '',
			        edit_url: '',
			        del_url: '',
			        multi_url: '',
			        table: 'flbooth_goods_comment',
			    }
			});
			...
			table.bootstrapTable({
			    ...
			    columns: [
			        [
			            ...
			            {field: 'status', title: __('Status'), searchList: {"normal":__('Normal'),"hidden":__('Hidden')}, formatter: Table.api.formatter.status},
			            {field: 'operate', title: __('Operate'), table: table, events: Table.api.events.operate,buttons: [{name: 'detail',title: __('查看评论'),classname: 'btn btn-xs btn-info btn-dialog',icon: 'fa fa-eye',url: 'flbooth/comment/detail'}],formatter: Table.api.formatter.operate}
			        ]
			    ]
			});
