define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function($, undefined, Backend,Table, Form) {
    ...
    var Controller = {
        deliver: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'flbooth/order/index' + location.search,
                    add_url: '',
                    edit_url: '',
                    del_url: 'flbooth/order/del',
                    multi_url: 'flbooth/order/multi',
                    table: 'flbooth_order',
                }
            });
            var table = $("#table");
			...
			$(document).on("click", ".detail[data-id]", function () {
			    Backend.api.open('flbooth/order/detail/id/' + $(this).data('id'), __('查看详情'),{area:['1200px', '780px']});
			});
			//查看退款
			...
			$(document).on("click", ".btn-delivery", function () {
				if($(this).data('id')){
					Backend.api.open('flbooth/order/delivery/ids/' + $(this).data('id'), __('发货'),{area:['1000px', '700px']});
				}else{
					Backend.api.open('flbooth/order/delivery/ids/' + Table.api.selectedids(table), __('批量发货'),{area:['1000px', '700px']});
				}
			});
			// 打印 & 批量打印订单 自动关闭窗口parent.Layer.closeAll();
			$(document).on("click", ".btn-invoice", function () {
				if($(this).data('id')){
					Backend.api.open('flbooth/order/invoice/ids/' + $(this).data('id'), __('查看发货单'),{area:['1100px', '750px']});
				}else{
					Backend.api.open('flbooth/order/invoice/ids/' + Table.api.selectedids(table), __('批量查看发货单'),{area:['1100px', '750px']});
				}
			});
			// 查询物流状态
			$(document).on("click", ".kuaidisub[data-id]", function () {
			    Backend.api.open('flbooth/order/relative/id/' + $(this).data('id'), __('快递查询'),{area:['800px', '600px']});
			});
			// 提交云面单
		...
		template: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'flbooth/freight/index' + location.search,
                    add_url: 'flbooth/freight/add',
                    edit_url: 'flbooth/freight/edit',
                    del_url: 'flbooth/freight/del',
                    multi_url: 'flbooth/freight/multi',
                    table: 'flbooth_shop_freight',
                }
            });
