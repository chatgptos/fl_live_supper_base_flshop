	...
	var Controller = {
		goods: function() {
			...
			Table.api.init({
				extend: {
					...
					add_url: 'flbooth/groups/goodsAdd',
					edit_url: 'flbooth/groups/goodsEdit',
						...
						{field: 'modified', title: __('modified'), operate:'RANGE', addclass:'datetimerange', formatter: Table.api.formatter.datetime},
						{
							...
							events: {
						        'click .btn-copy': function (e, value, row, index) {
						            Fast.api.close();
									parent.Fast.api.open(`flbooth/groups/goodsEdit/type/copy/ids/${row.id}`, "编辑 复制商品", {area: ['90%', '80%']});
						        },
						    }, formatter: function () {
		...
		goodsadd: function() {
			var vm = new Vue({
				...
				methods: {
					...
					getCategory(e){
						...
						// 查询类目属性
						Fast.api.ajax("flbooth.goods/attribute?id=" + this.categoryId, (data, ret) =>{
			...
			$(document).on("click", ".btn-send", function () {
				Backend.api.open('flbooth/config/index/type/mailing/', __('完善寄件人信息'), {
			...
			$(document).on("click", ".btn-return", function () {
				Backend.api.open('flbooth/config/index/type/return/', __('完善退货信息'), {
					callback:function(value){
						console.log(value);
					}
				});
			...
			$(document).on("click", ".btn-brand", function () {
				Backend.api.open('flbooth/brand/add/', __('申请品牌'), {
					callback:function(value){
						console.log(value);
					}
				});
			...
			$(document).on("click", ".btn-freight", function () {
				Backend.api.open('flbooth/freight/add', __('新建运费模板'), {
					callback:function(value){
						console.log(value);
					}
				});
			...
			$(document).on("click", ".btn-shopsort", function () {
				Backend.api.open('flbooth/shopsort/add', __('新建店铺分类'), {
					callback:function(value){
						console.log(value);
					}
				});
			...
			if(Config.isdialog){
				...
				Form.api.bindevent($("form[role=form]"), function (data, ret) {
				    setTimeout(function () {
				    	location.href = Fast.api.fixurl('flbooth.goods/index.html');
				    }, 500);
				});
			}
		},
		...
		groups: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'flbooth/groups/groups' + location.search,
                    add_url: '',
                    edit_url: '',
                    del_url: '',
                    multi_url: '',
							...
							buttons: [
								{
								    ...
								    icon: 'fa fa-users',
								    url: 'flbooth/groups/groupsDetail'
								}
						    ], 
							formatter: Table.api.formatter.operate
						}
