...
class User extends Api
{
	...
	{
	    parent::_initialize();
		$this->model = model('app\api\model\flshop\find\User');
	}
	
	/**
	 * 获取关注列表
	 *
	 * @ApiSummary  (flshop 发现接口获取关注列表)
	 * @ApiMethod   (GET)
	 * 
	 */
	 ...
	 * 获取关注商家列表
	 *
	 * @ApiSummary  (flshop 发现接口获取关注商家列表)
	 * @ApiMethod   (GET)
	 * 
	 */
	 ...
	 * 获取发现用户信息
	 *
	 * @ApiSummary  (flshop 发现接口获取发现用户信息)
	 * @ApiMethod   (POST)
	 * 
	 */
		...
		if ($this->request->isPost()) {
			...
			switch ($type) {
				case 'follow':
					...
					break; 
				case 'likes':
					$model = model('app\api\model\flshop\find\Like');
					$where_type = 'find_id';
					$my_type = 'praised';
					break;
				default:
		    ...
		    if ($model->where($where)->count() === 0) {
		        $model->save($where);
				if($type === 'likes'){
					$like = model('app\api\model\flshop\find\Find')->get($id);
					$like->setInc($type);
	...
	public function getTheme()
	{
		$row = $this->model
			...
			->find();
		$row['list'] = model('app\api\model\flshop\Theme')
	 ...
	 * 修改创作中心主题
	 *
	 * @ApiSummary  (flshop 发现接口修改创作中心主题)
	 * @ApiMethod   (POST)
	 * 
	 */
