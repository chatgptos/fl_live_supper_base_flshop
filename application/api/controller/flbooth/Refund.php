...
class Refund extends Api
{
	...
	public function refundList()
	{
		...
		if ($this->request->isPost()) {
			$list = model('app\api\model\flshop\Refund')
	...
	public function addApply()
	{
		...
		if ($this->request->isPost()) {
			...
			if($params['order_type'] === 'goods'){
				$orderModel = model('app\api\model\flshop\Order');
				$orderGoodsModel = model('app\api\model\flshop\OrderGoods');
			}else if($params['order_type'] === 'groups'){
				$orderModel = model('app\api\model\flshop\groups\Order');
				$orderGoodsModel = model('app\api\model\flshop\groups\OrderGoods');
			}
			// 查询商品
			...
			// 查询支付 1.1.2升级
			$pay = model('app\api\model\flshop\Pay')->get(['order_id' => $order->id, 'type' => $params['order_type']]);
			// 判断金额 1.0.2升级
	...
	public function getRefundInfo()
	{
		...
		// 获取快递列表
		$kuaidi = model('app\api\model\flshop\Kuaidi')
			->field('id,name,code')
			->select();
	 ...
	 * 提交关闭退款
	 *
	 * @ApiSummary  (flshop 退款接口提交关闭退款)
	 * @ApiMethod   (GET)
	 */
	public function closeRefund($id = null)
	{
		$row = model('app\api\model\flshop\Refund')
	 ...
	 * 获取退款历史
	 *
	 * @ApiSummary  (flshop 退款接口获取退款历史)
	 * @ApiMethod   (GET)
	 */
	public function getRefundLog($id = null)
	{
		$log = model('app\api\model\flshop\RefundLog')
		...
		if($log){
			...
			foreach ($log as $vo) {
				if($vo['type'] == 0){
					...
					$shop = model('app\api\model\flshop\Shop')->get($vo['shop_id']);
					$name = $shop['shopname'];
					$avatar = $shop['avatar'];
				}else if($vo['type'] == 2){
					$name = '小二';
					$avatar = '/assets/addons/flshop/img/common/service_3x.png';
				}else{
					$name = '系统';
					$avatar = '/assets/addons/flshop/img/common/logo.png';
				}
				$data[] = [
					'id' => $vo['id'],
					'content' => $vo['content'],
	...
	{
		return model('app\api\model\flshop\RefundLog')->save([
			'user_id' => $this->auth->id,
			'refund_id' => $refund_id,
			'content' => $content
		]);
