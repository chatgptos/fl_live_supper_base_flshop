...
class Flshop extends Controller
{
    ...
    public function _initialize()
    {
        ...
        if (!$this->auth->match($this->noNeedLogin)) {
			...
			$this->shopauth = \app\index\model\flshop\Auth::get(['user_id' => $this->auth->id]);
			if($this->shopauth){
				switch ($this->shopauth->verify) {
				    case 0:
						$this->error(__('审核信息尚未完善'),'index/flshop/entry');
				    case 1:
						$this->error(__('店铺尚未提交审核'),'index/flshop/entry');
					case 2:
						$this->error(__('店铺正在审核中'),'index/flshop/entry');
					case 4:
						$this->error(__('开通店铺未通过'),'index/flshop/entry');
				}
			}else{
				$this->error(__('尚未开通店铺'),'index/flshop/entry');
			}
        } else {
            // 如果有传递token才验证是否登录状态
            if ($token) {
		...
		$this->shop = \app\index\model\flshop\Shop::get(['user_id' => $this->auth->id]);
        ...
        $config = [
			...
			'socketurl'      => $flshop['ini']['socketurl'],
			'document'		 => $flshop['config']['shop_document'], //文档
			'qun'			 => $flshop['config']['shop_qun'], //QQ群
            'upload'         => $upload,
