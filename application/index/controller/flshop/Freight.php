            ...
            if ($params) {
                ...
                try {
					...
					if($params['isdelivery'] == 0){
						...
						if(!model('app\index\model\flshop\ShopFreightData')->allowField(true)->saveAll($list)){
							$result = false;
						}
					}
                    Db::commit();
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
					...
					if($params['isdelivery'] == 0){
						// 删除原来数据
						model('app\index\model\flshop\ShopFreightData')
						...
						$results = model('app\index\model\flshop\ShopFreightData')->allowField(true)->saveAll($list);
					}else{
						// 删除原来数据
						model('app\index\model\flshop\ShopFreightData')
							->where('freight_id','in',$ids)
