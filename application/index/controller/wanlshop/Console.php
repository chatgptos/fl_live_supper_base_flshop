...
class Console extends Wanlshop
{
    ...
    public function index()
    {
		...
		//七日增长
		$week = model('app\index\model\wanlshop\Order')->whereTime('created', 'week')->where('shop_id', $shop_id)->count();
		$lastweek = model('app\index\model\wanlshop\Order')->whereTime('created', 'last week')->where('shop_id', $shop_id)->count();
        ...
        $this->view->assign([
            ...
            'todayorder'       => model('app\index\model\wanlshop\Order')->whereTime('created', 'today')->where('shop_id', $shop_id)->count(), // 今日订单数量
            'unsettleorder'    => model('app\index\model\wanlshop\Order')->whereTime('created', 'today')->where(['shop_id' => $shop_id, 'state' => 2])->count(), // 未处理数量
            'sevendnu'         => $sevendnu.'%', 
            'sevendau'         => model('app\index\model\wanlshop\Pay')->whereTime('created', 'today')->where('shop_id', $shop_id)->sum('price'), //今日销售额
            'paylist'          => $paylist,
