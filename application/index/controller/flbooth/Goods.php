...
class Goods extends flbooth
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\index\model\flbooth\Goods;
        // 类目
        $tree = Tree::instance();
		// 1.0.2升级 过滤隐藏
    ...
    {
        return $this->view->fetch('flbooth/goods/index');
    }
	    ...
	    if ($this->request->isPost()) {
	        ...
	        if ($params) {
	            ...
	            try {
					...
					if(!model('app\index\model\flbooth\GoodsSpu')->allowField(true)->saveAll($spu)){
						$result == false;
					}
					// 写入SKU
					$sku = [];
					...
					if(!model('app\index\model\flbooth\GoodsSku')->allowField(true)->saveAll($sku)){
						$result == false;
					}
	                Db::commit();
    ...
    public function edit($ids = null)
    {
		...
		// 查询SKU
		$skuItem = model('app\index\model\flbooth\GoodsSku')
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
					...
					// 删除原来数据,重新写入SPU
					model('app\index\model\flbooth\GoodsSpu')
					...
					if(!model('app\index\model\flbooth\GoodsSku')->allowField(true)->saveAll($oldsku)){
						$result == false;
					}
					// 写入SKU
					$sku = [];
		...
		$spuData = model('app\index\model\flbooth\GoodsSpu')->all(['goods_id' => $ids]);
		$suk = [];
	...
	public function attribute()
	{
	    if ($this->request->isAjax()) {
	        ...
	        $list  = model('app\index\model\flbooth\Attribute')
