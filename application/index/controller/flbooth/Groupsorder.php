...
use addons\flbooth\library\WanlSdk\Ehund; //快递100订阅
 ...
 * @icon fa fa-circle-o
 */
class Groupsorder extends flbooth
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\index\model\flbooth\groups\Order;
        $kuaidi = new \app\index\model\flbooth\Kuaidi;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $row) {
				...
				$row->pay = model('app\index\model\flbooth\Pay')
    ...
    public function detail($id = null, $order_no = null)
    {
        ...
        $row['address'] = model('app\index\model\flbooth\groups\OrderAddress')
	...
	public function relative($id = null)
	{
		...
		$data = model('app\index\model\flbooth\KuaidiSub')
			->where(['express_no' => $row['express_no']])
			->find();
    ...
    public function delivery($ids = null)
    {
        ...
        foreach ($row as $vo) {
            ...
            $vo['address'] = model('app\index\model\flbooth\groups\OrderAddress')
        ...
        if ($this->request->isAjax()) {
			...
			foreach ($request['row']['order']['id'] as $key => $id) {
				...
				$is_express_no = model('app\index\model\flbooth\KuaidiSub')->where(['express_no' => $express_no])->count();
                // 订阅快递查询
            ...
            // 写入快递订阅列表
            if ($express) model('app\index\model\flbooth\KuaidiSub')->saveAll($express);
            $this->success();
        }
    ...
    {
        return $this->view->fetch('flbooth/comment/index');
    }
	...
	private function pushOrder($order_id = 0, $state = '已发货')
	{
		...
		$orderGoods = model('app\index\model\flbooth\groups\OrderGoods')
		...
		$notice = model('app\index\model\flbooth\Notice')->saveAll($msgData);
	}
}
