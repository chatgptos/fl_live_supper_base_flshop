...
class Groups extends flbooth
{
    ...
    public function _initialize()
    {
        parent::_initialize();
		$tree = Tree::instance();
		$tree->init(model('app\index\model\flbooth\Category')->where(['type' => 'goods', 'isnav' => 1])->field('id,pid,name')->order('weigh asc,id asc')->select());
		$this->assignconfig('channelList', $tree->getTreeArray(0));
		
		$this->groupsModel = new \app\index\model\flbooth\groups\Groups;
		$this->goodsModel = new \app\index\model\flbooth\groups\Goods;
		$this->goodsSpuModel = new \app\index\model\flbooth\groups\GoodsSpu;
		$this->goodsSkuModel = new \app\index\model\flbooth\groups\GoodsSku;
		$this->ladderModel = new \app\index\model\flbooth\groups\Ladder;
    }
	...
	public function goodsAdd()
	{
	    ...
	    $shop_id = $this->shop->id;
		// 判断是否存在品牌
		$row['brand'] = model('app\index\model\flbooth\Brand')->where(['state' => 1])->count();
		// 判断是否有店铺分类
		$row['shopsort'] = model('app\index\model\flbooth\ShopSort')->where('shop_id',$shop_id)->count();
		// 判断是否有运费模板
		$row['freight'] = model('app\index\model\flbooth\ShopFreight')->where('shop_id',$shop_id)->count();
		// 判断是否有寄件人信息
		$row['config'] = model('app\index\model\flbooth\ShopConfig')->where('shop_id',$shop_id)->find();
		// 打开方式
	...
	public function goodsEdit($ids = null, $type = null)
	{
		...
		// 判断业务类型
		if($type === 'copy'){
			$goodsModel = model('app\index\model\flbooth\Goods');
			$goodsSkuModel = model('app\index\model\flbooth\GoodsSku');
			$goodsSpuModel = model('app\index\model\flbooth\GoodsSpu');
		}else{
    ...
    public function groupsDetail($ids = null, $group_no = null)
    {
        ...
        $row->team = model('app\index\model\flbooth\groups\Team')
