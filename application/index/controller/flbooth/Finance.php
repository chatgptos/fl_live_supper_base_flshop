...
class Finance extends flbooth
{
    ...
    public function bill()
    {
		...
		// 设置模块
		$this->model = model('app\index\model\flbooth\Money');
    ...
    public function withdraw()
    {
        ...
        $this->request->filter(['strip_tags']);
		// 设置模块
		$this->model = model('app\index\model\flbooth\Withdraw');
        if ($this->request->isAjax()) {
            list($where, $sort, $order, $offset, $limit) = $this->buildparams();
            $total = $this->model
                ->where($where)
	...
	public function payment()
	{
		...
		if ($this->request->isPost()) {
			...
			// 查询提现账户
			$account = model('app\index\model\flbooth\PayAccount')
			...
			if ($config['withdraw']['monthlimit']) {
			    $count = model('app\index\model\flbooth\Withdraw')->where('user_id', $this->auth->id)->whereTime('created', 'month')->count();
			...
			try {
			    ...
			    $withdraw = model('app\index\model\flbooth\Withdraw')->create($data);
				$pay = new WanlPay;
			...
			$bank = model('app\index\model\flbooth\PayAccount')
	...
	public function user()
	{
		...
		$this->request->filter(['strip_tags']);
		// 设置模块
		$this->model = model('app\index\model\flbooth\PayAccount');
		if ($this->request->isAjax()) {
		    list($where, $sort, $order, $offset, $limit) = $this->buildparams();
		    $total = $this->model
		        ->where($where)
	...
	public function userAdd()
	{
	    ...
	    if ($this->request->isPost()) {
	        ...
	        if ($params) {
	            ...
	            try {
					$params['state'] = 0;
	                $result = model('app\index\model\flbooth\PayAccount')->allowField(true)->save($params);
	                Db::commit();
	...
	{
		// 设置模块
	    $row = model('app\index\model\flbooth\PayAccount')->get($ids);
	    if (!$row) {
	...
	{
		$row = model('app\index\model\flbooth\Withdraw')
			->where(['id' => $ids, 'user_id' => $this->auth->id])
			->find();
		if (!$row) {
		    $this->error(__('No Results were found'));
