 ...
 * @icon fa fa-circle-o
 */
class Refund extends flbooth
{
    protected $noNeedLogin = '';
    protected $noNeedRight = '*';
    /**
     * Refund模型对象
     * @var \app\index\model\flbooth\Refund
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\index\model\flbooth\Refund;
	...
	public function detail($ids = null, $order_id = null, $order_no = null)
	{
		...
		if($row['order_type'] === 'groups'){
			$row['ordergoods'] = model('app\index\model\flbooth\groups\OrderGoods')
				->where('id', 'in', $row['goods_ids'])
				->where('shop_id', $row['shop_id'])
				->select();
		}else{
			$row['ordergoods'] = model('app\index\model\flbooth\OrderGoods')
				->where('id', 'in', $row['goods_ids'])
				->where('shop_id', $row['shop_id'])
				->select();
		}
		$row['log'] = model('app\index\model\flbooth\RefundLog')
	...
	public function agree($ids = null)
	{
		...
		try {
			...
			if($row['type'] == 0){
				...
				if($row['order_type'] === 'groups'){
					// 查询订单是已确定收货
					$order = model('app\index\model\flbooth\groups\Order')->get($row['order_id']);
					// 订单状态:1=待支付,2=待成团,3=待发货,4=待收货,5=待评论,6=已完成,7=已取消
					$orderPay = $order['state'] == 5 ? true : false;
				}else{
					$order = model('app\index\model\flbooth\Order')->get($row['order_id']);
				...
				if($orderPay){
					// 扣商家
					controller('addons\flbooth\library\WanlPay\WanlPay')->money(-$row['price'], $order['shop']['user_id'], '确认收货，同意退款', 'refund', $order['order_no']);
				}
				// 退款给用户
				controller('addons\flbooth\library\WanlPay\WanlPay')->money(+$row['price'], $row['user_id'], '卖家同意退款', 'refund', $order['order_no']);
				...
				// 退货地址
				$shopConfig = model('app\index\model\flbooth\ShopConfig')
					->where(['shop_id' => $this->shop->id])
	...
	public function receiving($ids = null)
	{
		...
		try {
			...
			// 判断业务类型
			if($row['order_type'] === 'groups'){
				$orderModel = model('app\index\model\flbooth\groups\Order');
			}else{
				$orderModel = model('app\index\model\flbooth\Order');
			}
			// 查询订单是已确定收货
	...
	private function pushRefund($refund_id = 0, $order_id = 0, $goods_id = 0, $title = '', $order_type = 'goods')
	{
		if($order_type === 'groups'){
			$orderModel = model('app\index\model\flbooth\groups\Order');
			$orderGoodsModel = model('app\index\model\flbooth\groups\OrderGoods');
		}else{
			$orderModel = model('app\index\model\flbooth\Order');
			$orderGoodsModel = model('app\index\model\flbooth\OrderGoods');
		}
		...
		$notice = model('app\index\model\flbooth\Notice');
		$notice->data($msg);
	...
	{
		return model('app\index\model\flbooth\RefundLog')->allowField(true)->save([
			'shop_id' => $this->shop->id,
			'user_id' => $user_id,
			'refund_id' => $refund_id,
			'type' => 1,
