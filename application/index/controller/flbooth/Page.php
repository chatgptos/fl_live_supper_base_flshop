...
class Page extends flbooth
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\index\model\flbooth\Page;
	...
	public function link()
	{
	    ...
	    if ($this->request->isAjax()) {
			...
			if(!$filter || $filter == 'sort'){
				$sort = model('app\index\model\flbooth\ShopSort')
					->where(['shop_id' => $this->shop->id])
					->select();
				$sortData = [];
			...
			if(!$filter || $filter == 'page'){
				$page = model('app\index\model\flbooth\Page')
					->where(['shop_id' => $this->shop->id])
					->select();
				$pageData = [];
