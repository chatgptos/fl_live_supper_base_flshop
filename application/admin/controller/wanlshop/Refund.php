	...
	public function agree($ids = null)
	{
		...
		try {
			...
			if($row['type'] == 0){
				...
				controller('addons\wanlshop\library\WanlPay\WanlPay')->money(+$row['price'], $row['user_id'], '客服判定同意退款', 'refund', $row['order_no']);
				
				// 1.1.3升级
				$config = get_addon_config('wanlshop');
				if($config['config']['refund_switch'] == 'Y'){
					// 检查是否原路返还（不写到money防止自动任务时原路返回） 1.1.2升级
					controller('addons\wanlshop\library\WanlPay\WanlPay')->refund($row['id'], $row['price'], $row['order_pay_id']);
				}
				//后续版本推送订购单
				// ...
			}else if($row['type'] == 1){
